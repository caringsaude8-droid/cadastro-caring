...
export class UserFormModalComponent implements OnInit, OnChanges {
  ...
  formData: Partial<User> & { empresaId?: number; senha?: string; cpf?: string } = {
    nome: '',
    email: '',
    status: 'ativo',
    telefone: '',
  ...
  resetForm(): void {
    if (this.mode === 'edit' && this.user) {
      this.formData = {
        ...
        empresaId: this.user['empresaId'] || undefined,
        senha: '',
        cpf: this.user['cpf'] || ''
      };
    } else {
  ...
  onSubmit(): void {
    if (this.isFormValid()) {
      const userData: any = {
        ...
        perfil: (this.formData.perfil as string).toUpperCase(),
        telefone: this.formData.telefone!,
        empresaId: this.formData.empresaId,
